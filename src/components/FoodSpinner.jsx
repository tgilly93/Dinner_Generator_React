...
const FoodSpinner = () => {
  ...
  const slices = foodIcons.map((icon, index) => {
    ...
    const pathData = `M ${center} ${center} 
            L ${x1} ${y1}
            A ${radius} ${radius} 0 ${largeArcFlag} 1 ${x2} ${y2}
            Z`;

    const iconX =
      center + radius * 0.65 * Math.cos(startingAngle + anglePerSlice / 2);
    const iconY =
      center + radius * 0.65 * Math.sin(startingAngle + anglePerSlice / 2);

    return (
      <g key={index}>
        <path
          d={pathData}
          fill={`hsl(${(index * 360) / numSlices}, 70%, 60%)`}
        />
        <text
          x={iconX}
          y={iconY}
